    ...
    const string &outputDir)
{
    ...
    for (auto imagePath : images)
    {
        ...
        for (const T &sigma : sigmas)
        {
            ...
            double psnr = 0;//computePSNR<T>(inputImage, noisyImage, maxVal);
            double mssim = 0;//computeMSSIM<T>(inputImage, noisyImage, maxVal);
...
void runTests()
{
    ...
    const string imageDir("../../../images/polyu/");
    const string outputDir("../../../output/polyu/");
    ...
    vector<T> sigmas = {40};
    vector<int> searchRadii = {10};
    vector<int> neighborRadii = {3};
