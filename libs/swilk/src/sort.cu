...
__host__ __device__ void SwilkSort::heapSort(double arr[], int N)
{
...
__device__ void SwilkSort::bubbleSort(double arr[], int N)
{
    for (int i = 0; i < N; i++)
    {
        for (int j = i + 1; j < N; j++)
...
// insertion sort
__device__ void SwilkSort::insertionSort(double arr[], int n)
{
    double key;
    int i, j;
    for (i = 1; i < n; i++)
