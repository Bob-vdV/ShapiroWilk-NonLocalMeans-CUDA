...
#define CREATE_HISTOGRAMS(type, buffer)                                                \
	/* Clear counters */                                                               \
	ZeroMemory(mHistogram, 256 * 4 * sizeof(uint));                                    \
                                                                                       \
	/* Prepare for temporal coherence */                                               \
	type PrevVal = (type)buffer[mIndices[0]];                                          \
	bool AlreadySorted = true; /* Optimism... */                                       \
	uint *Indices = mIndices;                                                          \
                                                                                       \
	/* Prepare to count */                                                             \
	ubyte *p = (ubyte *)input;                                                         \
	ubyte *pe = &p[nb * 4];                                                            \
	uint *h0 = &mHistogram[0];	 /* Histogram for first pass (LSB)	*/                  \
	uint *h1 = &mHistogram[256]; /* Histogram for second pass		*/                      \
	uint *h2 = &mHistogram[512]; /* Histogram for third pass			*/                      \
	uint *h3 = &mHistogram[768]; /* Histogram for last pass (MSB)	*/                   \
                                                                                       \
	while (p != pe)                                                                    \
	{                                                                                  \
...
#define CHECK_PASS_VALIDITY(pass)                                                           \
	...
	uint *CurCount = &mHistogram[pass << 8];                                                \
                                                                                            \
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
__device__ void radixSort(const uint *input, uint nb, bool signedvalues)
{
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
__device__ void radixSort(const float *input2, uint nb)
{
	// Checkings
	if (!input2 || !nb)
		return *this;
	...
	for (uint j = 0; j < 4; j++)
	{
		...
		else
		{
			...
			if (PerformPass)
			{
				...
				mOffset[255] = 0;
				for (i = 0; i < 127; i++)
					mOffset[254 - i] = mOffset[255 - i] + CurCount[255 - i]; // Fixing the wrong order for negative values
				for (i = 128; i < 256; i++)
				...
				for (i = 0; i < nb; i++)
				{
					uint Radix = input[mIndices[i]] >> 24; // Radix byte, same as above. AND is useless here (uint).
