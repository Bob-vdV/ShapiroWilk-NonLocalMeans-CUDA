...
__global__ void kernel(const T *in, const double *a, T *out, int rows, int cols, double sigma)
{
    ...
    __shared__ T searchWindow[searchDiam * searchDiam];
...
void swnlmcuda(const Mat &noisyImage, Mat &denoised, const T sigma)
{
    ...
    cudaDeviceSetLimit(cudaLimitMallocHeapSize, (size_t)4 * 1024 * 1024 * 1024); // Set to 4 GB
    //const size_t sharedMemSize = (2 * padding + 1) * (2 * padding + 1) * sizeof(T);

    kernel<T, searchRadius, neighborRadius><<<blocks, threads>>>(d_in, d_a, d_out, rows, cols, sigma);
